...
## P-020 — Applied R-023's memory fix to the Dockerfile; live memory usage came in far under R's measurement

**Date:** 2026-08-18 · **Status:** Accepted
**Context:** R (R-023) wired their memory-fix work (R-021 SQLite chunk lookup, R-022 torch-free
`LiteE5Embedder`) into the real production path and published two new release artifacts
(`embedder-lite-onnx-v1`, `index-metadata_aware-v2`), specifying the exact 3 Dockerfile steps
needed, but explicitly leaving the deploy decision itself to me/the user — even after this work,
R's own isolated measurement was 727MB against a 512MB budget, still short. Presented this
honestly to the user (real progress, real remaining gap, real options) rather than either quietly
deploying something likely to crash the working demo or quietly doing nothing. User chose: try
deploying anyway, with the previous known-good (stub-serving) commit as the rollback plan.
**Applying the fix found two more real issues, neither anticipated by R's writeup:**
1. **A tarball naming mismatch**: `embedder-lite-onnx-v1`'s release asset extracts to a top-level
   `multilingual-e5-small-lite/` directory, not `multilingual-e5-small/` (what `LiteE5Embedder`'s
   `DEFAULT_ONNX_MODEL_DIR` expects) — found by downloading and inspecting the tarball's actual
   contents before writing the extraction path, not by assuming the naming pattern from the index
   release. Fixed with `tar --strip-components=1`, extracting directly into the target directory
   regardless of the tarball's own internal folder name.
2. **A Python version floor**: the Dockerfile's base image (`python:3.11-slim`) is one version
   below what `numpy>=2.5` (part of the new `retrieval-lean` extra) requires (3.12+) — never
   surfaced before because retrieval extras were never actually installed in the image until this
   change. Bumped to `python:3.12-slim`.
**Verification, live-relevant not just local:** built and ran the fixed image locally with Docker's
own `-m 512m` memory limit — matching Render's actual free-tier constraint, not just eyeballing a
number. Result: **446.8MiB steady-state, ~456MiB after several more requests** — comfortably under
512MB, nowhere near R's isolated 727MB figure. Also confirmed *real* (non-stub) retrieval end to
end: `retrieve` timing 1072ms (vs. the stub's ~0.007ms), and a live G3 borderline refusal
(`"Top result confidence 0.88 is below threshold 0.8835"`) — a genuine real-world instance of
exactly the close-call scenario R-017's calibration fix was written for, not a synthetic test case.
**Why the discrepancy from R's 727MB is plausible, not alarming:** R's measurement was a raw Python
process on Windows (`tasklist`), loading the retriever directly outside any container. This test
measures the actual deployed artifact (the real Docker image, the real FastAPI/uvicorn app, Linux
inside a cgroup-enforced memory limit) — closer to the real question ("does the actual deployment
fit") than a same-language-different-OS-different-process-shape proxy measurement. Different
malloc/allocator behavior between Windows and Linux, and the overhead of a bare interpreter vs. a
full ASGI stack, are both plausible, ordinary explanations — not a sign either measurement was
wrong for what it was actually measuring.
**Consequences:** Deploying live next. If Render's actual free-tier container behaves like this
local test, real retrieval should now be live without needing the paid-tier fallback at all —
would fully resolve `docs/RISKS.md` R4/R-R21. If Render's environment differs enough to still OOM
(a real possibility — cloud container limits and enforcement can differ from local Docker Desktop
in ways not fully visible from here), the rollback plan (redeploy the previous stub-serving commit)
is ready and low-cost. Either outcome will be a real, live-verified answer, not another projection.
