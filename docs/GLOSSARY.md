...
200ms budget is even achievable with in-process search instead of a linear scan.

**RRF (Reciprocal Rank Fusion)** — how we combine a dense-search ranked list and a sparse
(BM25) ranked list into one ranking, without needing to normalize two incomparable score scales
(cosine similarity vs. BM25 score). Each document gets `1 / (k + rank)` from each list it appears in
(k=60 here), and the scores are summed. Simple, robust, and — per the 2026 literature — the
production default for exactly this kind of hybrid retrieval.
...
low-priority stop rather than being late to everyone after it.
